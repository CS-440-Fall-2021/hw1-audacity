...
# helper funct to fill image according to onChannel
...
def blow(path):
    ...
    fillImage(originalImage, fImg, (0, 1, 0))  # 0,1,0 = keep green channel only
...
# Selecting color to use for plotting point according to closest vertex
def select_color(current_pos: (int, int), p1: (int, int), p2: (int, int),\
                 p3: (int, int)) -> (int, int, int):
